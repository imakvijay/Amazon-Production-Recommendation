...
class ResizeTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        X = self.cv2.cvtColor(X, self.cv2.COLOR_BGR2RGB)
...
def cosine_similarity(similar_images_df):
    cosine_df = pd.read_csv('../../Modelling/Binfiles/cosine_similarity.csv', index_col=0)
